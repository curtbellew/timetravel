				...
				'validate': function (value) {
					if (isNaN(value) || value.length != 5) {
						throw new oj.ValidatorError(
							"Error", "Zip code value is incorrect.");//'Zip code format incorrect: '+value+'.  The value must be a number that is 5 digits long eg (80831).');
					}
				},
			}];
			//3.3.1
			self.emailMatchValidator = [{
				'validate': function (value) {
					...
					if (firstEmail !== secondEmail) {
						throw new oj.ValidatorError(
							"Error", "Error");//"Email fields do not match. "+firstEmail+' = '+secondEmail);
					}
				},
			}];
